...
- `SettingsScreen.kt` — Sign out (with OAuth revocation dialog), read-only repo display, auth type label ("Connected via GitHub"/"Connected via PAT"), two-step digital assistant setup, delete all data
...
- `SettingsViewModel.kt` — Settings state, sign out with OAuth token revocation, role check, delete all data, auth type display
